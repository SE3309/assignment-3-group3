...
def create_table(file_name: str, num_tuples: int,
    ...
    # write the sql statements to load all csvs into the tables
    write_sql(file_name)
    if(not new):
        return
    ...
    # if ck, generate guarenteed ck relations
    ck_index: list[int] = []
...
# generate guarenteed candidate key combinations
...
def gen_ck(ck_list: list[str], fk_columns: dict[str, list[str]], callable_list: list[Callable[[], object]], ck_callables: list[int]):
    # each column will have 2^#_of_columns elements 
    # e.g. 
